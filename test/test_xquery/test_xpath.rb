...
class TestXPath < Test::Unit::TestCase
  ...
  TEST_CASES = [
    ...
    TestCase.new("doc(  \"catalog.xml\"  )/catalog/product[colorChoices][number='784']/number",
      ["<number>784</number>"]),
    TestCase.new("doc(  \"catalog.xml\"  )/catalog/product[colorChoices][number<='558']/number",
      ["<number>557cdata cast<></number>"]),
    TestCase.new("doc(  \"catalog.xml\"  )/catalog/product[colorChoices]/number",
      ['<number>557cdata cast<></number>', '<number>784</number>']),
    ...
    TestCase.new("doc(  \"catalog.xml\"  )/catalog/product[nocdata]/number[. >= 563]",
    ...
    TestCase.new("doc(  \"catalog.xml\"  )/catalog/product/number[. <= '563']",
      ["<number>557cdata cast<></number>", "<number>563</number>", "<number>443</number>"]),
    TestCase.new("doc(  \"catalog.xml\"  )/catalog/product/number[. < '563']",
      ["<number>557cdata cast<></number>", "<number>443</number>"]),
    TestCase.new("doc(\"catalog.xml\")/catalog/product[nocdata][number = 443]/@dept",
      ["ACC"]), 
    TestCase.new("doc(\"catalog.xml\")/catalog/product[nocdata][number =443]/@dept",
      ["ACC"]),
    TestCase.new("doc(\"catalog.xml\")/catalog/product[nocdata][number= 443]/@dept",
    ...
    TestCase.new("doc(\"catalog.xml\")/catalog/product[nocdata][number=443]/@dept",
      ["ACC"]),
    TestCase.new("doc(\"catalog.xml\")//product[nocdata][number=443]/@dept",
      ["ACC"]),
    TestCase.new("doc(\"catalog.xml\")/catalog/*[nocdata][number=443]/@dept",
      ["ACC"]),
    ...
    TestCase.new("doc(\"catalog.xml\")//product[last()]",
      ["<product dept=\"MEN\"><number>784</number><name language=\"en\">Cotton Dress Shirt</name><colorChoices>white gray</colorChoices><desc>Our<i>favorite</i>shirt!<!--Second commentary--></desc><nocdata></nocdata></product>"]),
